#!/usr/bin/env python3
"""
Project Vanguard-0 Standalone Evaluation Harness.
Executes AI Kavach on the 4 Impossible-Class Nightmare targets.
Generates an independent report: reports/vanguard_nightmare_report.md
"""
...
from tests.vanguard_nightmare.manifest import VANGUARD_TARGETS

console = Console()

def run_vanguard_suite():
    setup_logging(verbose=False)
    console.print("\n[bold red]🔥 Starting Project Vanguard-0: Nightmare Vulnerability Gauntlet[/bold red]\n")
    ...
    for idx, target in enumerate(VANGUARD_TARGETS, 1):
        console.print(f"[bold cyan][{idx}/{len(VANGUARD_TARGETS)}] Target: {target.id}[/bold cyan]")
        console.print(f"  Description: {target.description}")
        console.print(f"  CWE: {target.cwe_id} | Complexity: {target.complexity.upper()}")
    ...
    # Print Summary Table
    table = Table(title="Project Vanguard-0 Nightmare Results")
    table.add_column("Target ID", style="cyan")
    table.add_column("CWE", style="magenta")
    table.add_column("Status", style="bold green")
    table.add_column("Winning Agent", style="yellow")
    table.add_column("Hardening", style="blue")
    ...
    console.print(
        f"\n[bold]Passed all configured gates: {passed}/{len(VANGUARD_TARGETS)}[/bold] | "
    ...
    # Save Vanguard Report
    report_dir = PROJECT_ROOT / "reports"
    report_dir.mkdir(exist_ok=True)
    report_path = report_dir / "vanguard_nightmare_report.md"
    
    md = f"""# 🛡️ Project Vanguard-0: Nightmare Vulnerability Evaluation Report
...
"""
    for r in results:
        ...
        res = r["result"]
        if res is None:
            md += f"| **{t.id}** | {t.cwe_id} | {t.description} | **SKIPPED** | — | {r['hardening']} | — |\n"
            continue
        pov_note = "" if r["preflight"].pov_usable else " *(no valid PoV — not dynamically proven)*"
        md += (f"| **{t.id}** | {t.cwe_id} | {t.description}{pov_note} | "
               f"**{res.status.upper()}** | {res.winning_agent or 'None'} | "
               f"{r['hardening']} | {res.elapsed_seconds:.2f} |\n")
...
"""
    ...
    console.print(f"📄 Saved Vanguard Nightmare Report to: [bold]{report_path}[/bold]\n")
